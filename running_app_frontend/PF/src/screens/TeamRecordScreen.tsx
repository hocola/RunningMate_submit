...
type TeamRecordRouteProp = RouteProp<RootStackParamList, 'TeamRecord'>;
type NavigationProp = NativeStackNavigationProp<RootStackParamList, 'TeamRecord'>;
...
interface RecordWithMember extends RunningRecordResponse {
...
const TeamRecordScreen = () => {
  const navigation = useNavigation<NavigationProp>();
  const route = useRoute<TeamRecordRouteProp>();
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      switch (activeTab) {
        case '주간':
          data = await runningApi.getRoomWeeklyRecords(roomId, formatDateForApi(weekStart));
          break;
        case '월간':
          data = await runningApi.getRoomMonthlyRecords(
            roomId,
          ...
          break;
        case '연간':
          data = await runningApi.getRoomYearlyRecords(roomId, currentDate.getFullYear());
          break;
        case '전체':
          data = await runningApi.getRoomRecords(roomId);
          break;
      }

      const recordsWithMember: RecordWithMember[] = data.map(record => {
      ...
      const filteredRecords = selectedMemberId !== null
        ? recordsWithMember.filter(r => r.userId === selectedMemberId)
        : recordsWithMember;
  ...
  }, [roomId, activeTab, weekStart, currentDate, members, selectedMemberId]);

  useEffect(() => {
    if (members.length > 0) {
      fetchRecords();
    }
  }, [fetchRecords, members]);
  ...
  const renderRecordItem = ({ item }: { item: RecordWithMember }) => (
    <TouchableOpacity
      ...
      <View style={styles.recordHeader}>
        <View style={styles.recordHeaderLeft}>
          <View style={styles.memberBadge}>
            <Ionicons name="person" size={12} color="#1E90FF" />
            <Text style={styles.memberBadgeText}>{item.memberNickname}</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>팀 기록</Text>
      ...
      >
        ...
        {members.map((member) => (
          <TouchableOpacity
            key={member.userId}
            style={[styles.memberChip, selectedMemberId === member.userId && styles.memberChipActive]}
            onPress={() => setSelectedMemberId(member.userId)}
          >
            <Text style={[styles.memberChipText, selectedMemberId === member.userId && styles.memberChipTextActive]}>
              {member.nickname}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtext}>팀원들과 함께 러닝을 시작해보세요!</Text>
        </View>
      )}
    </SafeAreaView>
  );
...
export default TeamRecordScreen;
